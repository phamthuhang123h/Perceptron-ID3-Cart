...
from sklearn.model_selection import train_test_split   #Chia mảng hoặc ma trận thành các tập con thử nghiệm và huấn luyện ngẫu nhiên. 
...
clf = tree.DecisionTreeClassifier(criterion = "gini")     #Sử dụng chỉ số “gini index” làm thước đo trong khi tạo cây quyết định 
clf = clf.fit(X_train, y_train)         #huấn luyện mô hình dựa trên tập x_train, y_train

#đưa ra giá trị dự đoán trên dữ liệu đầu vào là X_test
y_predict = clf.predict(X_test)

count = 0       #lấy biến đếm
print("Thực tế\t\tDự đoán")
for i in range(0,len(y_predict)):       #tạo vòng lặp từ 0 đến số lượng phẩn tử của y_test
